...
async def start(message: types.Message):
    await bot.send_message(message.from_user.id, 'Привет {0.first_name}!\nЭтот бот преднозначен для отправки прогноза погоды на день.\nНастоятельно  прошу использовать специальную клавиатура, дабы бот работал корректно, спасибо! '.format(message.from_user), reply_markup=nav.cityMenu)
    await bot.send_message(message.from_user.id, 'Выбери город, в котором ты живешь!')
...
async def bot_message(message: types.Message):
    if message.text =='Lera':
        ...
        if (db.subscriber_exists(message.from_user.id)):
            ...
            await bot.send_message(message.from_user.id, "Вы не установлии свой город, это можно сдлеать в меню 'Настройки'")       
