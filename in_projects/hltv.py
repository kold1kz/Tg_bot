...
class Hltv:
    ...
    def new_games(self):
        ...
        new=[]
        items =html.select('.title > .items > .item >a')
        for i in items:
    ...
    def game_info(self, url):
        ...
        poster = re.match(r'background-image:\s*url\((.+?)\)', html.select('.image-game-log0 >.image')[0]['style'])
        ...
        info = {
            ...
            "score" : self.identify_score(html.select(" .game-hltv-score > .score")[0]['class'][1]),
            "excerpt" : html.select('.article.article-show')[0].text[0:200]+'...'
        }

        return info

    def indetify_score(self, score):
        if(score =="score-1"):
            return "Мусор"
        elif(score == "score-2"):
            return "Проходняк"
    ...
    def download_image(selfself,url):
    ...
    def get_lastkey(self):
        ...
        items = html.select('.title > .items > .items >a')
